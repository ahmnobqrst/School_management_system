            ...
            @foreach ($children as $index => $child)
                <li class="nav-item">
                    <a class="nav-link {{ $index == 0 ? 'active' : '' }}" data-toggle="tab"
                        href="#child-{{ $child->id }}">
                        <i class="fa-solid fa-user-graduate"></i> {{ $child->name }}
                    </a>
                </li>
            @endforeach
        </ul>
        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                <div class="tab-content">
                    @foreach ($children as $index => $child)
                        <div class="tab-pane fade {{ $index == 0 ? 'show active' : '' }}" id="child-{{ $child->id }}">
                                            ...
                                            <tr>
                                                ...
                                                @for ($p = 1; $p <= $maxPeriods; $p++)
                                                    <td class="align-middle p-2"
                                                        style="background: #fafafa; min-width: 160px;">
                                                        @if (isset($childrenSchedules[$child->id][$day][$p]))
                                                            @foreach ($childrenSchedules[$child->id][$day][$p] as $item)
