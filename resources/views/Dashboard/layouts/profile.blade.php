            ...
            <form action="{{ route('admin.update.profile', $admin->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="profile-header">
                    ...
                    <img id="profileImagePreview"
                        src="{{ $admin->image && file_exists(public_path('storage/'.$admin->image)) 
            ? asset('storage/'.$admin->image) 
            : asset('images/user.jpeg') }}"
                        onclick="document.getElementById('profileImageInput').click();">

                    <h4 class="mt-3">{{ $admin->name }}</h4>
                    <p class="text-light mb-0">{{ $admin->email }}</p>
                    <small>{{ __('Students_trans.admin') }}</small>
                </div>
                @csrf
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label>{{ __('Students_trans.Admin_name_ar') }}</label>
                    </div>
                    <div class="col-md-8">
