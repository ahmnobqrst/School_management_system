            ...
            <form action="{{ route('update.profile', $teacher->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="profile-header">
                    ...
                    <img id="profileImagePreview"
                        src="{{ $teacher->image && file_exists(public_path('storage/'.$teacher->image)) 
            ? asset('storage/'.$teacher->image) 
            : asset('images/user.jpeg') }}"
                        onclick="document.getElementById('profileImageInput').click();">

                    <h4 class="mt-3">{{ $teacher->name }}</h4>
                    <p class="text-light mb-0">{{ $teacher->email }}</p>
                    <small>{{ __('Students_trans.Teacher') }}</small>
                </div>
                @csrf
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label>{{ __('Students_trans.Teacher_name_ar') }}</label>
                    </div>
                    <div class="col-md-8">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label>{{ __('Students_trans.Teacher_name_en') }}</label>
                    </div>
                    <div class="col-md-8">
