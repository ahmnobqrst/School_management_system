...
<div class="container mt-5">
    <div class="profile-card">
        ...
        <div class="profile-body">
            <form action="{{ route('student.update.profile', $student->id) }}" method="post" enctype="multipart/form-data">

                <div class="profile-header">
                    <input type="file" id="profileImageInput" name="image" accept="image/*" style="display:none;">
                    <img id="profileImagePreview"
                        src="{{ $student->image && file_exists(public_path('storage/'.$student->image)) 
            ? asset('storage/'.$student->image) 
            : asset('images/user.jpeg') }}"
                        onclick="document.getElementById('profileImageInput').click();">

                    <h4 class="mt-3">{{ $student->name }}</h4>
                    <p class="text-light mb-0">{{ $student->email }}</p>
                    <small>{{ __('Students_trans.student') }}</small>
                </div>
                @csrf
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label>{{ __('Students_trans.student_name_ar') }}</label>
                    </div>
                    <div class="col-md-8">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label>{{ __('Students_trans.student_name_en') }}</label>
                    </div>
                    <div class="col-md-8">
